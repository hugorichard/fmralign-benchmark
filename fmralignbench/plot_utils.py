...
METHODS_LABELS_DICT = {
    ...
    "mvica_26": "MultiView\nICA",
    "mvica_21": "MultiView\nICA",
    "mvica_26_basc_444": "MVICA",
    "mvica_21_basc_444": "MVICA",
    "amvica_26_basc_444": "ShICA",
    "amvica_21_basc_444": "ShICA",
...
def parse_score(
    ...
    path_to_fit_timings = os.path.join(
        decoding_dir,
        "{}_{}_{}_on_{}_fit_timings.csv".format(
            decoding_task, roi_code, method, alignment_data_label
        ),
    )
    path_to_overhead_timings = os.path.join(
        decoding_dir,
        "{}_{}_{}_on_{}_overhead_timings.csv".format(
            decoding_task, roi_code, method, alignment_data_label
        ),
    )
...
def draw_plot(ax, data, positions, labels, edge_color, fill_color):
    bp = ax.boxplot(
        ...
        showmeans=True,
        patch_artist=True,
        meanprops=dict(linestyle="-", linewidth=1),
...
def make_diff(
    decoding_task, methods, roi_code, alignment_data_label, decoding_dir
):
    scores = fetch_scores(
        decoding_task, alignment_data_label, roi_code, decoding_dir, methods
    )
    scores_diff = np.vstack(
        [scores[:, i] - scores[:, 0] for i in range(1, np.shape(scores)[1])]
    )
    return scores_diff
...
def plot_one_method(ax, data, position, method_width, cmap, median=False):
    n_datasets = len(data)
    offsets = np.linspace(
        -method_width / 2 + 0.1, method_width / 2 - 0.1, n_datasets
    )
    ...
    for dataset, offset, c in zip(data, offsets, colors):
        n = len(dataset)
        ax.plot(dataset, position * np.ones(n) + offset, "o", color=c, ms=4)
    pass
...
def make_perf_sub(
    ...
    y_lims,
    cmap,
    x_ticks=[-0.05, -0.02, 0, 0.02, 0.05, 0.08],
    x_lims=(-0.08, 0.10),
    title="Decoding accuracy improvement",
):
...
def make_time_sub(ax, methods_times, positions, method_width, y_lims, cmap):
    ...
    ax.xaxis.set_major_formatter(mtick.FormatStrFormatter("x%.2f"))
    ...
    ax.set_title(
        "Relative computation time", fontsize=16, fontname="Helvetica"
    )
    pass
...
def make_score_diffs(
    datasets_list, methods, ROI, add_srm, root_dir, fit_times=True
):
    score_diffs, times = [], []
    for task in datasets_list:
        ...
        score_diffs.append(
            make_diff(
                task, methods_, roi_code, alignment_data_label, decoding_dir
            )
        )
...
def make_smoothing_figure():
    ...
    smoothing_methods = [
        ...
        "smoothing_10",
        "smoothing_05",
        "pairwise_scaled_orthogonal_schaefer_300",
    ]
    ...
    plt.legend(
        handles=legends_points,
        loc=(1.2, 0.5),
        fancybox=True,
        fontsize=14,
        title="Decoding tasks",
    )
    if not os.path.isdir(os.path.join(ROOT_FOLDER, "figures")):
        os.mkdir(os.path.join(ROOT_FOLDER, "figures"))
    plt.savefig(
        os.path.join(ROOT_FOLDER, "figures", "supplementary_3.png"),
        bbox_inches="tight",
    )
    pass
...
def make_bench_figure(ROI):
    ...
    methods = [
        "anat_inter_subject",
        "intra_subject_ridge_cv_schaefer_1000",
        "HArad_5_sparse_3",
        "pairwise_scaled_orthogonal_schaefer_300",
        "pairwise_ot_e-1_schaefer_300",
    ]
    ...
    normalized_times = [
        [s[l] / np.mean(swapped_time[ref_index][l]) for l in range(len(s))]
        for s in swapped_time
    ]
    cmap, colors, positions, y_lims = plot_props(len(methods_) - 1)
    ...
    if ROI:
        ...
        roi_ = ""
    x_ticks = [-0.2, -0.1, -0.05, 0, 0.05, 0.10, 0.2, 0.3]
    ...
    make_perf_sub(
        ...
        x_ticks=x_ticks,
        x_lims=x_lims,
        title="{}Decoding accuracy improvement".format(roi_),
    )
    plt.yticks(positions, labels, fontsize=14, fontname="sans-serif")
    ax1 = plt.subplot(gs[1])
    make_time_sub(
        ax1, normalized_times, positions, METHOD_WIDTH, y_lims, colors
    )
    legends_points = []
    for dataset, c in zip(DATASET_LIST, colors):
        legends_points.append(
    ...
    if ROI:
        plt.savefig(
            os.path.join(ROOT_FOLDER, "figures", "experiment2.png"),
            bbox_inches="tight",
        )
    else:
        plt.savefig(
            os.path.join(ROOT_FOLDER, "figures", "experiment1.png"),
            bbox_inches="tight",
        )
    pass
...
def make_within_subject_decoding_figure():
    ...
    make_perf_sub(
        ...
        x_ticks=[0, 0.10, 0.20],
        x_lims=(-0.10, 0.25),
        title="Within - Inter-subject",
    )
    plt.yticks(
        positions, ["" for i in positions], fontsize=14, fontname="sans-serif"
    )
    legends_points = []
    for dataset, c in zip(DATASET_LIST, colors):
        legends_points.append(
    ...
    plt.savefig(
        os.path.join(
            ROOT_FOLDER, "figures", "experiment_1_within_decoding.png"
        ),
        bbox_inches="tight",
    )
    pass
...
def make_supplementary1_roi_minus_fullbrain_figure():
    ...
    methods_data = [
        [
            np.hstack(
                fetch_dataset_roi(methods, True, task, ROOT_FOLDER)
                - fetch_dataset_roi(methods, False, task, ROOT_FOLDER)
            )
            for task in DATASET_LIST
        ]
        for methods in [["pairwise_scaled_orthogonal_schaefer_300"]]
    ]
    ...
    for meth_data, position in zip(methods_data, positions):
        plot_one_method(
            ax, np.asarray(meth_data), position, METHOD_WIDTH, colors
        )
        ax.set_xticks([-0.1, -0.05, 0, 0.05])
        ax.xaxis.set_major_formatter(
            mtick.PercentFormatter(xmax=1, decimals=0)
        )
        ax.set_xlim(left=-0.15, right=0.08)
        ax.tick_params(axis="x", labelsize=14)
    ...
    plt.savefig(
        os.path.join(
            ROOT_FOLDER, "figures", "supplementary_1_roi_minus_fullbrain.png"
        ),
        bbox_inches="tight",
    )
    pass
...
def make_supplementary4_surface_volumic_figure():
    ...
    fullbrain_roi_diff = [
        np.hstack(
            ...
            - fetch_dataset_roi(
                ["anat_inter_subject"], False, task, ROOT_FOLDER
            )
        )
    ]
    surf_fullbrain_roi_diff = [
        np.hstack(
            fetch_dataset_roi(
            ...
            - fetch_dataset_roi(
                ["anat_inter_subject"], False, task, ROOT_FOLDER, surf=True
            )
        )
    ]
    ...
    for meth_data, position in zip(methods_data, positions):
        ...
        ax.set_xticks([-0.1, -0.05, 0, 0.05, 0.10, 0.2])
        ax.xaxis.set_major_formatter(
            mtick.PercentFormatter(xmax=1, decimals=0)
        )
        ax.tick_params(axis="x", labelsize=14)
    ...
    plt.savefig(
        os.path.join(
            ROOT_FOLDER,
            "figures",
            "supplementary_4_surfacic_vs_volumetric.png",
        ),
        bbox_inches="tight",
    )
    pass
